...
- [synthpop](https://cran.r-project.org/web/packages/synthpop/synthpop.pdf) is a "tool for producing synthetic versions of microdata containing confidential information so that they are safe to be released to users for exploratory analysis". This library contains the code for calculating *pMSE*-ratio. 
...
Waiting for approval from NIST PSCR to provide the original and the competitors' differentially private synthetic data. 
...
- `utility.gen()` from `synthpop` calculates the *pMSE*-ratio and other utility metrics.
